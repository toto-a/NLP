...
class BilingualDatasets(Dataset) :
    ...
    def __getitem__(self, index) :
        ...
        enc_num_padding_tokens=self.seq_len- len(enc_input_token) -2 #SOS and EOS  token
        dec_num_padding_tokens=self.seq_len- len(dec_input_token) -1 #SOS toke
        ...
        #What we expect from the decoder
        label=torch.cat([
            torch.tensor(dec_input_token,dtype=torch.int64),
            self.eos_token,
            torch.tensor([self.pad_token]* dec_num_padding_tokens, dtype=torch.int64)
